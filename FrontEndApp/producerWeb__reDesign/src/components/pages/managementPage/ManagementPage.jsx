...
export default function ManagementPage() {
  ...
  const makeFarmList = () => {
    return Object.entries(farm).map(([key, value]) => (
      <FarmCard key={key} farmName={key} farmValue={value} setSelectedFarm={setSelectedFarm} selectedProperty={selectedProperty} />
    ));
  }
  ...
  return (<>
    <div className="
    "> {/* wrapper */}
      <div className="h-[7vh] w-screen flex items-center pb-[1vh]"> {/* header */}
        <ManagementHeader/>
      </div>
      <div className=" flex  w-screen h-[93vh] "> {/* content section */}
        <div className="
        w-[15vw]
        h-[93vh]
        overflow-y-auto
        ...
        scrollbar-hide
        "> {/* aside */}
          <div className="bg-teal-200 h-[95%] m-4 rounded-3xl"> {/* aside wrapper, Rounded Box */}
            {makeFarmList()}
          </div>
        </div>
        <div className="
        ...
        scrollbar-hide
        " > {/* content */}
          <div className="h-[95%] m-4  flex flex-col"> {/* content wrapper, Rounded Box */}
            <div className=" flex justify-around h-[5%] rounded-t-2xl bg-teal-300"> {/* tab section */}
              <button> Prop One</button>
              <button> Prop Two</button>
              <button> Prop Three</button>
              <button> Prop Four</button>
            </div>
            <div className="h-[95%] rounded-b-2xl bg-teal-200"> {/* content section */}
              <ManagementContent selectedFarm={selectedFarm} />
            </div>
          </div>
